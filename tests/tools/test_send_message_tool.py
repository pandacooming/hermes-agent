...
class TestParseTargetRefDiscord:
    ...
    def test_discord_parent_channel_negative_snowflake(self):
        """Discord negative snowflake IDs (used for channels) work without thread."""
        chat_id, thread_id, is_explicit = _parse_target_ref("discord", "-1003724596514")
        assert chat_id == "-1003724596514"
        assert thread_id is None
        assert is_explicit is True

    def test_discord_negative_snowflake_with_thread(self):
